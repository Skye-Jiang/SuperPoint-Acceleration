...
class Coco(BaseDataset):
    ...
    def _init_dataset(self, **config):
        base_path = Path(DATA_PATH, 'COCO/train2014/') #/event_image_left/
    ...
    def _get_data(self, files, split_name, **config):
        ...
        def _preprocess(image):
            if config['preprocessing']['rgb2grey']:
        ...
        #data_1 = tf.data.Dataset.zip({'image': images, 'name': names})
        ...
        if has_keypoints:
            ...
            #data_1 = tf.data.Dataset.zip((data_1, kp)).map(
                    #lambda d, k: {**d, 'keypoints': k})
            #data_1 = data_1.map(pipeline.add_dummy_valid_mask)
        ...
        if config['warped_pair']['enable']:
            ...
            data_1 = data.map_parallel(pipeline.imadjust)
            data = data.map_parallel(pipeline.imadjust)
            warped = data_1.map_parallel(lambda d: pipeline.homographic_augmentation(
                d, add_homography=True, **config['warped_pair']))
            #warped = warped.map_parallel(pipeline.imadjust)
