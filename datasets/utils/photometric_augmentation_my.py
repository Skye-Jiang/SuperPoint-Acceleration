...
def additive_gaussian_noise(image, stddev_range=[5, 95]):
    # a.设定参数sigma 和 Xmean-->如何选定偏移和标准差数值[5,95]
    # b.产生一个高斯随机数
    # c.根据输入像素计算出输出像素
...
def random_contrast(image, strength_range=[0.5, 1.5]):
    ...
    #在区间[lower, upper]-->[0.5, 1.5]中随机选取的contrast_factor,通过随机因子调整图像的对比度
    #返回：对比度调整张量
    #对比度：图像暗和亮的落差值，即图像最大灰度级和最小灰度级之间的差值
    return tf.clip_by_value(tf.image.random_contrast(image, *strength_range), 0, 255)

#plt.imshow(contrast.eval())#eval实现list、dict、tuple与str之间的转化，同样str函数把list，dict，tuple转为为字符串

# def salt_pepper_noise(image, salt_vs_pepper=0.2, amount=0.004):
#
#     def _salt_pepper_noise(image, salt_vs_pepper, amount):
#         row, col, _ = image.shape
#         num_salt = np.ceil(amount * image.shape[0] * salt_vs_pepper)
#         num_pepper = np.ceil(amount * image.shape[0] * (1.0 - salt_vs_pepper))
#     	# Add Salt noise
...
#         return image
#
#     salted = tf.py_func(_salt_pepper_noise, [image], tf.float32)
#     return salted
