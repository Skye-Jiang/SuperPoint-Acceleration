...
#noise, rotation, Homograph
#已经有处理后的tensor
#如何显式并存储到指定路径
#生成的图片是否是三值，每一帧的是否是整数，由于处理的图像由RGB[255,255,255]-->events[255,0,255]
...
def plot_imgs(imgs, titles=None, cmap='brg', ylabel='', normalize=False, ax=None, dpi=100):
    ...
    for i in range(n):
        ...
        cv2.waitKey(0)
        cv2.imwrite('/home/chenyu/darkpoint_tf/superpoint/photometric_img/origin.jpg', imgs[0]*255)
        if titles:
...
config = {
    'labels': '/home/shared_data/chenyu_eventcamera/exper_darkpoint/outputs/magic-point_cocoevent-export1',
